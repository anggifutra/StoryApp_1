...
class UploadActivity : AppCompatActivity() {
    ...
    private fun uploadImage() {
        val getDesc = binding.edAddDescription.text.toString()
        if (getFile != null && getDesc != "") {
            ...
            viewModel.errorMessage.observe(this){
                if (it.equals("Story created successfully")){
                    val i = Intent(this@UploadActivity, ListStoryActivity::class.java)
                    startActivity(i)
                }else{
                    showToast("Upload Failed : $it")
                }
            }

        } else {
            Toast.makeText(this@UploadActivity, "Silakan masukkan berkas gambar dan deskripsi terlebih dahulu.", Toast.LENGTH_SHORT).show()
        }
    }
